...
const Page = ({navigation}) => {
  ...
  const [isAnonymous, setIsAnonymous] = useState(user.incognito);
  ...
  useEffect(() => {
    ...
    user.photos.map(userPhoto => {
      photos.map((photo, index) => {
        if (userPhoto.order === photo.id) {
          photo.filled = true;
          photo.photo = userPhoto.photoUrl;

          photosAux[index] = photo;
        }
      });
    });
        ...
        <ContentArea>
          ...
          <SwitchButtonsArea>
            <SwitchButton
              label="Modo anônimo"
              value={!isAnonymous}
              setValue={() => setIsAnonymous(!isAnonymous)}
